...
public enum CivicInformationEndpoint: Endpoint {
    ...
    func asURLRequest() -> URLRequest {
        ...
        params: if let parameters = parameters {
            ...
            urlQueryItems = dict.map { item -> [URLQueryItem] in
                if let array = item.value as? Array<Any> {
                    var items: [URLQueryItem] = []
                    array.forEach { items.append(URLQueryItem(name: item.key, value: String(describing: $0))) }
                    return items
                }
            ...
            urlComps.queryItems = urlQueryItems.flatMap { $0 }
        }
